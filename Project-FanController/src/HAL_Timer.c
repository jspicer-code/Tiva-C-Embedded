...
int Timer_EnableTimerPeriodic(TimerBlock_t block, uint32_t interval, uint8_t priority, PFN_TimerCallback callback)
{
	...
	switch (block) {
		...
		case TIMER0:
			...
			NVIC_PRI4_R = (NVIC_PRI4_R & 0x00FFFFFF) | (priority << 28);
		
			Timer0_Callback = callback;

			// Enable...
			NVIC_EN0_R |= 0x00080000; // bit 19
		...
		case TIMER1:
			...
			// The priority bits (value 0-7) occupy the upper nibble of the priority byte.
			NVIC_PRI5_R = (NVIC_PRI5_R & 0xFFFF00FF) | (priority << 12);
		...
		case TIMER2:
			...
			// Register 34: Interrupt 20-23 Priority (PRI5), offset 0x414
			// The priority bits (value 0-7) occupy the upper nibble of the priority byte.
			NVIC_PRI5_R = (NVIC_PRI5_R & 0x00FFFFFF) | (priority << 28); // priority 3
