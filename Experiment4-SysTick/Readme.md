...
## Overview
This experiment tests the SysTick timer and PLL on the EV-TMC4123GLX board.  The PLL is enabled and configured to run the bus clock at 80MHz.  The preset for the SysTick timer is taken from the value of 8 GPIO inputs that are connected to an 8-pin DIP switch.  The binary value on the DIP switches is converted to decimal and multiplied by 10ms.  The maximum timer delay is therefore 255 * 10ms = 2.55 seconds.  
